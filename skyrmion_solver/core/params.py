"""
Core theory agnostic parameter management for skyrmion_solver.
...
class Params:
    ...
    lsz : float or None, optional
        Grid spacing along the z direction.
    ...
    unit_length : bool, optional
        Flag enabling a unit length constraint.
...
class ResolvedParams:
    ...
    ylen : int
        Number of lattice points along the y direction.
    \len : int
        Number of lattice points along the \ direction.
    halo : int
        Halo width used by finite difference stencils.
    ...
    unit_length : int
        Integer flag enabling a unit length constraint.
    xsize : float
        Physical size of the domain along the x direction.
    ysize : float
        Physical size of the domain along the y direction.
    zsize : float
        Physical size of the domain along the z direction.
    lsx : float
        Grid spacing along the x direction.
    lsy : float
        Grid spacing along the y direction.
    ...
    def from_params(p: Params) -> "ResolvedParams":
        ...
        lsz = p.lsz if p.lsz is not None else p.zsize / (p.zlen - 1)
        grid_volume = lsx * lsy * lsz
        time_step = p.time_step if p.time_step is not None else (p.courant * lsx)
        dim_grid = p.xlen * p.ylen * p.zlen
        dim_fields = int(p.number_total_fields) * dim_grid
        return ResolvedParams(xlen=p.xlen, ylen=p.ylen, zlen = p.zlen, halo=p.halo, number_coordinates=p.number_coordinates, number_total_fields=int(p.number_total_fields), dim_grid=dim_grid, dim_fields=dim_fields, killkinen=1 if p.killkinen else 0, newtonflow=1 if p.newtonflow else 0, unit_length=1 if p.unit_length else 0, xsize=float(p.xsize), ysize=float(p.ysize), zsize=float(p.zsize), lsx=float(lsx), lsy=float(lsy), lsz=float(lsz), grid_volume=float(grid_volume), time_step=float(time_step))
...
def pack_device_params(rp: ResolvedParams):
    ...
    """
    p_i = np.array([rp.xlen, rp.ylen, rp.zlen, rp.halo, rp.number_coordinates, rp.number_total_fields, rp.dim_grid, rp.dim_fields, rp.killkinen, rp.newtonflow, rp.unit_length], dtype=np.int32)
    p_f = np.array([rp.xsize, rp.ysize, rp.zsize, rp.lsx, rp.lsy, rp.lsz, rp.grid_volume, rp.time_step], dtype=np.float64)
    return p_i, p_f