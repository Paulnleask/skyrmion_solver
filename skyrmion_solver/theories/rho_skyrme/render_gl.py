...
Purpose
-------
Provide the rho-Skyrme CUDA/OpenGL ray-marching renderer that keeps the visualization volume on the GPU.

Usage
-----
Use ``GLRenderer`` to render the rho-Skyrme simulation interactively.
...
from skyrmion_solver.theories.rho_skyrme.observables import compute_skyrmion_number
...
DISPLAY_TITLES = {
    DISPLAY_ENERGY_SINGLE: "Rho-Skyrme: Energy density",
    DISPLAY_ENERGY_RUNGE: "Rho-Skyrme: Energy density (Runge)",
    DISPLAY_RHO_MESON: "Rho-Skyrme: Rho-meson density"
}
...
class GLRenderer:
    """
    Interactive renderer for the rho-Skyrme simulation.
    ...
    def __init__(self, width: int, height: int, depth: int, title: str = "rho_skyrme (ray-march)") -> None:
        """
    ...
    def _on_key(self, window, key: int, scancode: int, action: int, mods: int) -> None:
        ...
        if key == glfw.KEY_F3:
            self.set_display_mode(DISPLAY_RHO_MESON)
            return
    ...
    def _update_window_title(self) -> None:
        ...
        """
        self.backend.set_window_title(DISPLAY_TITLES.get(self.display_mode, "rho_skyrme"))
...
def _compute_density_for_mode(sim, mode: int) -> None:
    ...
    """
    if mode == DISPLAY_RHO_MESON and hasattr(sim, "compute_rho_meson_density"):
