"""
Initial condition and ansatz CUDA kernels for rho_skyrme simulations.

Examples
--------
Use ``create_ground_state_kernel`` to initialize the vacuum state.
Use ``create_initial_configuration_kernel`` to write a hedgehog or rational-map ansatz into the field.
Use ``create_rational_map_skyrmion_kernel`` to write a translated and rotated rational-map Skyrmion into a temporary field.
Use ``product_ansatz_kernel`` to multiply a temporary Skyrmion field into an accumulated field.
Use ``initialize`` as the theory entrypoint for field initialization, including ``mode="smorgasbord"``.
"""
...
from skyrmion_solver.theories.rho_skyrme.kernels import compute_norm_skyrme_field
...
def create_rational_map_skyrmion_kernel(
    ...
    zz0 = math.cos(phi) * t
    zz1 = math.sin(phi) * t
    f = profilefun(r, bint, profile_m0, profile_m1)
...
def _initialize_smorgasbord(
    *,
    ...
    Notes
    -----
    This routine follows the attached reference at a high level by repeatedly generating a single rational-map factor and multiplying it into the accumulated field. :contentReference[oaicite:2]{index=2}
    ...
    count = int(config.get("count", max(1, int(round(abs(float(config.get("baryon_number", p_f_h[10] if p_f_h is not None else 1.0))))))))
...
def initialize(
    *,
    ...
    Notes
    -----
    If ``config`` does not provide overrides, the ansatz defaults are read from the packed Skyrme parameter arrays.
    The packed Skyrme ABI is assumed to satisfy ``p_i[11:14] = (hedgehog, rational_map, uniform)`` and ``p_f[10:12] = (baryon_number, isospin_rotation)``.
    ...
    baryon_number = float(cfg.get("baryon_number", p_f_h[11] if p_f_h is not None else 1.0))
    isospin_rotation = float(cfg.get("isospin_rotation", p_f_h[12] if p_f_h is not None else 0.0))
