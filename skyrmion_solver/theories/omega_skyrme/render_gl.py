...
Purpose
-------
Provide the omega-Skyrme CUDA/OpenGL ray-marching renderer that keeps the visualization volume on the GPU.

Usage
-----
Use ``GLRenderer`` to render the omega-Skyrme simulation interactively.
...
class GLRenderer:
    ...
    def __init__(self, width: int, height: int, depth: int, title: str = "omega_skyrme (ray-march)") -> None:
        """
    ...
    def _on_key(self, window, key: int, scancode: int, action: int, mods: int) -> None:
        ...
        if key == glfw.KEY_F3:
            self.set_display_mode(DISPLAY_OMEGA)
            return
    ...
    def _update_window_title(self) -> None:
        ...
        """
        self.backend.set_window_title(DISPLAY_TITLES.get(self.display_mode, "omega_skyrme"))
...
def _compute_density_for_mode(sim, mode: int) -> None:
    ...
    """
    if mode == DISPLAY_OMEGA and hasattr(sim, "compute_potential_density"):
