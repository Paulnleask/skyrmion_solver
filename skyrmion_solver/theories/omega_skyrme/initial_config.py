"""
Initial condition and ansatz CUDA kernels for omega_skyrme simulations.
...
from skyrmion_solver.theories.omega_skyrme.kernels import compute_norm_skyrme_field, _chern_simons_current_point
...
def _initialize_smorgasbord(
    *,
    ...
    create_omega_field_kernel[grid3d, block3d](Field, d1fd1x, p_i_d, p_f_d)


@cuda.jit
def create_omega_field_kernel(Field, d1fd1x, p_i, p_f):
    """
    Initialize the omega field from the local Chern-Simons current.
    ...
    p_i : device array
        Integer parameter array.
    p_f : device array
        Float parameter array containing the theory parameters.

    Returns
    -------
    None
        The omega component ``Field[idx_field(4, x, y, z, p_i)]`` is written in place.

    Notes
    -----
    This matches the C++ initialization
    ``Omega_Field[idx] = -c_omega * ChernSimonsCurrent(Skyrme_Field, x, y, z)``.
    The Skyrme first derivatives are recomputed locally before evaluating the Chern-Simons current.
    """
    x, y, z = cuda.grid(3)
    if not in_bounds(x, y, z, p_i):
        return
    ...
    Field[idx_field(4, x, y, z, p_i)] = -c_omega * _chern_simons_current_point(Field, d1fd1x, x, y, z, p_i)
