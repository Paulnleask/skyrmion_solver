...
Purpose
-------
Provide the nuclear Skyrme CUDA/OpenGL ray-marching renderer that keeps the visualization volume on the GPU.
...
class GLRenderer:
    ...
    def __init__(self, width: int, height: int, depth: int, title: str = "nuclear_skyrme (ray-march)") -> None:
        """
    ...
    def _on_key(self, window, key: int, scancode: int, action: int, mods: int) -> None:
        ...
        if key == glfw.KEY_U:
            U = self._sim.compute_U_tensor()
            _print_matrix("U", U)
            return

        if key == glfw.KEY_V:
            V = self._sim.compute_V_tensor()
            _print_matrix("V", V)
            return

        if key == glfw.KEY_W:
            W = self._sim.compute_W_tensor()
            _print_matrix("W", W)
            return

        if key == glfw.KEY_Q:
            Q = self._sim.compute_quadrupole_tensor()
            _print_matrix("Q", Q)
            print(f"Tr(Q) = {float(np.trace(Q)): .2f}")
            return

        if key == glfw.KEY_D:
            D = self._sim.compute_D_term()
            print(f"D(0) = {D: .2f}")
            return
    ...
    def _update_window_title(self) -> None:
        ...
        """
        self.backend.set_window_title(DISPLAY_TITLES.get(self.display_mode, "nuclear_skyrme"))
