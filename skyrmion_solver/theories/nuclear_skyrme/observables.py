"""
Host side observable wrappers for energy, baryon number, centre of mass, RMS radius, inertia tensors, quadrupole tensor, and D-term.
...
Use ``compute_U_tensor``, ``compute_V_tensor``, and ``compute_W_tensor`` to evaluate the inertia tensors.
Use ``compute_quadrupole_tensor`` to evaluate the intrinsic electric quadrupole tensor.
Use ``compute_D_term`` to evaluate the monopole D-term form factor.
"""
...
from skyrmion_solver.theories.nuclear_skyrme.kernels import compute_energy_kernel, compute_skyrmion_number_kernel
from skyrmion_solver.theories.nuclear_skyrme.kernels import compute_center_of_mass_kernel, compute_rms_radius_kernel
from skyrmion_solver.theories.nuclear_skyrme.kernels import compute_U_tensor_kernel, compute_V_tensor_kernel, compute_W_tensor_kernel
from skyrmion_solver.theories.nuclear_skyrme.kernels import compute_quadrupole_tensor_kernel, compute_D_integrand_kernel
...
def compute_U_tensor(Field, d1fd1x, obs9, tensor_scratch, entmp, gridsum_partial, p_i_d, p_f_d, p_i_h) -> np.ndarray:
    """
    ...
    d1fd1x : device array
        Device first derivative buffer.
    obs9 : device array
        Device tensor buffer of shape ``9 * dim_grid``.
    tensor_scratch : device array
        Device scalar scratch array.
    entmp : device array
        Device reduction scratch array.
    ...
    """
    grid3d, block3d = launch_3d(p_i_h, threads=(8, 8, 4))
    compute_U_tensor_kernel[grid3d, block3d](obs9, Field, d1fd1x, p_i_d, p_f_d)
    cuda.synchronize()
    return _reduce_tensor9(obs9, tensor_scratch, entmp, gridsum_partial, p_i_h)

def compute_V_tensor(Field, grid, d1fd1x, obs9, tensor_scratch, entmp, gridsum_partial, obs4, com_d, p_i_d, p_f_d, p_i_h) -> np.ndarray:
    """
    ...
    grid3d, block3d = launch_3d(p_i_h, threads=(8, 8, 4))
    compute_V_tensor_kernel[grid3d, block3d](obs9, grid, com_d, Field, d1fd1x, p_i_d, p_f_d)
    cuda.synchronize()
    return _reduce_tensor9(obs9, tensor_scratch, entmp, gridsum_partial, p_i_h)

def compute_W_tensor(Field, grid, d1fd1x, obs9, tensor_scratch, entmp, gridsum_partial, obs4, com_d, p_i_d, p_f_d, p_i_h) -> np.ndarray:
    """
    ...
    grid3d, block3d = launch_3d(p_i_h, threads=(8, 8, 4))
    compute_W_tensor_kernel[grid3d, block3d](obs9, grid, com_d, Field, d1fd1x, p_i_d, p_f_d)
    cuda.synchronize()
    return _reduce_tensor9(obs9, tensor_scratch, entmp, gridsum_partial, p_i_h)

def compute_quadrupole_tensor(Field, grid, d1fd1x, obs9, tensor_scratch, entmp, gridsum_partial, obs4, com_d, p_i_d, p_f_d, p_i_h) -> np.ndarray:
    """
    Compute the intrinsic isospin-0 electric quadrupole tensor.
    ...
    grid3d, block3d = launch_3d(p_i_h, threads=(8, 8, 4))
    compute_quadrupole_tensor_kernel[grid3d, block3d](obs9, grid, com_d, Field, d1fd1x, p_i_d, p_f_d)
    cuda.synchronize()
    return _reduce_tensor9(obs9, tensor_scratch, entmp, gridsum_partial, p_i_h)

def compute_D_term(Field, grid, d1fd1x, en, entmp, gridsum_partial, obs4, tensor_scratch, com_d, p_i_d, p_f_d, p_i_h, p_f_h) -> float:
    """
    Compute the monopole D-term form factor at zero momentum transfer.
    ...
    grid3d, block3d = launch_3d(p_i_h, threads=(8, 8, 4))
    compute_D_integrand_kernel[grid3d, block3d](en, grid, com_d, Field, d1fd1x, p_i_d, p_f_d)
    cuda.synchronize()
    integral = _sum_scalar_field(en, entmp, gridsum_partial, int(p_i_h[6]))
    return -(2.0 / 5.0) * E * integral