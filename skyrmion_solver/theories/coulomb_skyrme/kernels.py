...
Purpose
-------
Core CUDA kernels and device helpers for the three dimensional Coulomb gauged SU(2) Skyrme model with the standard pion mass potential.
...
def compute_energy_point(Field, d1fd1x, x, y, z, p_i, p_f):
    ...
    The implementation assumes the theory specific parameters are appended to the core float array as ``mass = p_f[8]`` and ``kappa = p_f[9]``.
    ...
    potential = Field[idx_field(4, x, y, z, p_i)]
    ...
    fxc = d1fd1x[idx_d1(0, 4, x, y, z, p_i)]
    ...
    fyc = d1fd1x[idx_d1(1, 4, x, y, z, p_i)]
    ...
    fzc = d1fd1x[idx_d1(2, 4, x, y, z, p_i)]
    ...
    energy = nx + ny + nz
    energy += (nx * ny - dxy * dxy) + (nx * nz - dxz * dxz) + (ny * nz - dyz * dyz)
    energy += 2.0 * mass2 * (1.0 - s0)
    # Coulomb 
    energy += 0.5 * kappa * (fxc * fxc + fyc * fyc + fzc * fzc)
    # energy += kappa * potential * _chern_simons_current_point(Field, d1fd1x, x, y, z, p_i)
...
def compute_potential_kernel(en, Field, p_i, p_f):
    """
    Compute the per site Coulomb potential density contributions across the grid.
    ...
    -------
    None
        The local Coulomb potential density contributions are written into ``en`` in place.

    Examples
    --------
    Launch ``compute_potential_kernel[grid3d, block3d](en, Field, p_i, p_f)`` to evaluate the Coulomb potential density on the grid.
    """
    x, y, z = cuda.grid(3)
    if not in_bounds(x, y, z, p_i):
        return
...
def do_gradient_step_point(Velocity, Field, EnergyGradient, d1fd1x, d2fd2x, x, y, z, p_i, p_f):
    ...
    for mu in range(number_skyrme_fields):
        EnergyGradient[idx_field(mu, x, y, z, p_i)] = 2.0 * g[mu]
    ...
    # Coulomb
    d_V0 = d1fd1x[idx_d1(0, 4, x, y, z, p_i)]
    d_V1 = d1fd1x[idx_d1(1, 4, x, y, z, p_i)]
    d_V2 = d1fd1x[idx_d1(2, 4, x, y, z, p_i)]
    ...
    for mu in range(number_skyrme_fields):
        ...
        for i in range(number_coordinates):
            d_V_i = d_V0 if i == 0 else (d_V1 if i == 1 else d_V2)
            ...
            for j in range(number_coordinates):
                for k in range(number_coordinates):
                    ...
                    g[mu] -= kappa / four_pi2 * eijk * d_V_i * cof
    ...
    # Coulomb gradient
    dd_V00 = d2fd2x[idx_d2(0, 0, 4, x, y, z, p_i)]
    dd_V11 = d2fd2x[idx_d2(1, 1, 4, x, y, z, p_i)]
    dd_V22 = d2fd2x[idx_d2(2, 2, 4, x, y, z, p_i)]

    g_V = 0.0
    g_V -= kappa * (dd_V00 + dd_V11 + dd_V22) + kappa * _chern_simons_current_point(Field, d1fd1x, x, y, z, p_i)

    EnergyGradient[idx_field(4, x, y, z, p_i)] = g_V
