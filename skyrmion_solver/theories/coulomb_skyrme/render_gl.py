...
Purpose
-------
Provide the Coulomb Skyrme CUDA/OpenGL ray-marching renderer that keeps the visualization volume on the GPU.

Usage
-----
Use ``GLRenderer`` to render the Coulomb Skyrme simulation interactively.
...
from skyrmion_solver.theories.coulomb_skyrme.observables import compute_skyrmion_number
...
DISPLAY_TITLES = {
    DISPLAY_ENERGY_SINGLE: "Coulomb Skyrme: Energy density",
    DISPLAY_ENERGY_RUNGE: "Coulomb Skyrme: Energy density (Runge)",
    DISPLAY_COULOMB: "Coulomb Skyrme: Coulomb potential density"
}
...
class GLRenderer:
    ...
    def __init__(self, width: int, height: int, depth: int, title: str = "coulomb_skyrme (ray-march)") -> None:
        """
    ...
    def _on_key(self, window, key: int, scancode: int, action: int, mods: int) -> None:
        ...
        if key == glfw.KEY_F3:
            self.set_display_mode(DISPLAY_COULOMB)
            return
    ...
    def _update_window_title(self) -> None:
        ...
        """
        self.backend.set_window_title(DISPLAY_TITLES.get(self.display_mode, "coulomb_skyrme"))
...
def _compute_density_for_mode(sim, mode: int) -> None:
    ...
    """
    if mode == DISPLAY_COULOMB and hasattr(sim, "compute_potential_density"):
