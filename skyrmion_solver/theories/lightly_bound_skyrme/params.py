"""
Lightly bound Skyrme theory specific parameters, parameter resolution, device packing, and terminal parameter documentation.

Examples
--------
Use ``default_params`` to create a lightly bound Skyrme model parameter set with optional overrides.
...
class Params(CoreParams):
    """
    User facing three dimensional lightly bound Skyrme model parameters.
    ...
    alpha : float, optional
        Coefficient controlling the lightly boundedness strength.
    ...
    Use ``p = Params(mpi=0.5, alpha=1.0, baryon_number=1.0, ansatz="hedgehog")`` to set model parameters and the initial ansatz.
...
def pack_device_params(rp: ResolvedParams):
    ...
    The Skyrme kernels assume ``mpi = p_f[8]`` and ``alpha = p_f[9]``.
...
def describe() -> None:
    ...
    """
    print("Field content:")
    print("  The three dimensional lightly bound Skyrme model uses 4 field components by default.")
    ...
    print("  alpha : coefficient controlling the lightly boundedness")
    print()
    ...
    print("  Theory float parameters are appended in the order: mpi, alpha, baryon_number, isospin_rotation.")
    print("  The Skyrme kernels therefore read mpi from p_f[8] and alpha from p_f[9].")
    print()